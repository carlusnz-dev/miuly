...
| **RF-005** | `finance` | Gestão de Contas Bancárias (`Bank`) | Cadastro e listagem de contas concretas (ex.: "Nubank corrente", "Carteira") por perfil, com moeda ISO 4217, nome único por perfil e saldo inicial (`balance`) armazenado com precisão decimal exata (`Numeric(10,2)`); o saldo atual é calculado a partir dos lançamentos pagos. |
| **RF-006** | `finance` | Registro de Lançamentos Financeiros (`Finance`) | Permite criar lançamentos de Entrada (`Inflow`), Saída (`Outflow`) e Transferência (`Transfer`), vinculados a uma conta bancária (`Bank`), valor na moeda da conta e categoria (`Tag`); lançamentos sem `paidAt` são previstos e não entram no saldo. |
| **RF-007** | `finance` | Transferências entre Contas Bancárias | Processa transferências em uma única linha associando conta de origem e conta de destino da mesma moeda, de forma atômica sem duplicar receitas ou despesas nas estatísticas. |
