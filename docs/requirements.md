...
| **RF-005** | `finance` | Gestão de Contas Bancárias (`Bank`) | Cadastro e listagem de contas concretas (ex.: "Nubank corrente", "Carteira") por perfil, com nome único por perfil e saldo próprio (`balance`) armazenado com precisão decimal exata (`Numeric(10,2)`). |
| **RF-006** | `finance` | Registro de Lançamentos Financeiros (`Finance`) | Permite criar lançamentos de Entrada (`Inflow`), Saída (`Outflow`) e Transferência (`Transfer`), vinculados a uma conta bancária (`Bank`), valor, moeda ISO 4217 e categoria (`Tag`). |
| **RF-007** | `finance` | Transferências entre Contas Bancárias | Processa transferências entre contas bancárias (`Bank`) afetando as duas pontas (origem/destino) de forma atômica sem duplicar receitas ou despesas nas estatísticas. |
